            ...
            try
            {
                switch (codec)
                {
                    case 1: //Handshake
                        ...
                        {
                            if (Handshake.NodeListeningPort == 5433)
                            {
                                trn.log.Log(new WarningLogEntry()
                                {
                                    LogType = WarningLogEntry.eLogType.DEBUG,
                                    Description = $"{trn.port}> 测试 handshake from {this.Handshake.NodeListeningPort}"
                                });
                            }
                        }
                    ...
                    case 10:
                        var member = TcpMsgMember.BiserDecode(data);
                        if (member != null)
                        {

                            switch (member.MemberCmdType)
                            {
                                case MemberCmdType.Add:
                                    trn.spider.AddMemberToClusterEndPoints(member.Host, member.Port);
                                    break;
                                case MemberCmdType.Remove:
                                    trn.spider.RemoveMember(member.Host, member.Port);
                                    break;
                                case MemberCmdType.Get:
                                    trn.spider.GetMembers(member.Host, member.Port);
                                    break;
                                case MemberCmdType.Response:
                                    trn.spider.AddPeers(member.ClusterEndPoints);
                                    break;
