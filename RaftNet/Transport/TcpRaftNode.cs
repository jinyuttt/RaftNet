...
namespace Raft.Transport
{
    ...
    {       
        ...
        public event Action<string, long, long> Handler;
        ...
        public TcpRaftNode(NodeSettings nodeSettings, string dbreezePath, Func<string, ulong, byte[], bool> OnCommit, int port = 4250, IWarningLog log = null)
        {
            ...
            foreach (var re_settings in this.NodeSettings.RaftEntitiesSettings)
            {
                ...
                rn.Handler += Rn_Handler;

                rn.NodeStart();
            }
        }

        private void Rn_Handler(string arg1, long arg2, long arg3)
        {
            if(Handler!=null)
            {
        ...
        private static void Rn_Handler1(string arg1, long arg2, long arg3)
        {
        ...
        /// <param name="port"></param>
        /// <returns></returns>
        public bool AddNewMember(string host, int port)
        {
           
            spider.SendToAllMemmbers(host, port);
            return false;
        ...
        /// 新增节点加入
        /// </summary>
        /// <param name="host">已经运行的节点IP</param>
        /// <param name="port">已经运行的节点端口</param>
        /// <returns></returns>
        public async Task AddSelfClusterEndPoints(string host, int port)
        {
           int peer=Interlocked.Increment(ref spider._peerCount);
            await  spider.SendToMemmberAsync(host, port);
            await Task.Run(() =>
              {
                  while (spider._peerCount < peer)
                  {
                      spider.SendToAllMemmbers("127.0.0.1", port);
                      Thread.Sleep(1000);
                  }
